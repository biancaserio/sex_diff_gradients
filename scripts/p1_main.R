...
lm.gsp.sex_age_icv <- function(df_dv, df_iv) {
...
# run model
GSP_lm_G1_res = lm.gsp.sex_age_icv(df_dv = GSP_array_aligned_G1, df_iv = GSP_demographics_cleaned)
GSP_lm_G2_res = lm.gsp.sex_age_icv(df_dv = GSP_array_aligned_G2, df_iv = GSP_demographics_cleaned)
GSP_lm_G3_res = lm.gsp.sex_age_icv(df_dv = GSP_array_aligned_G3, df_iv = GSP_demographics_cleaned)


# number of significant parcels
sum(GSP_lm_G1_res$q_val_sex < 0.05, na.rm=TRUE)  # other way: length(which(GSP_lm_G1_res$q_val_sex < 0.05))
sum(GSP_lm_G2_res$q_val_sex < 0.05, na.rm=TRUE)  
sum(GSP_lm_G3_res$q_val_sex < 0.05, na.rm=TRUE)  



lmer.sex_age_icv_ctrl_related
...
lm_G1_sex_age_icv_ctrl_related_res = lmer.sex_age_icv_ctrl_related(df_dv = HCP_array_aligned_G1, df_iv = HCP_demographics_cleaned)
lm_G2_sex_age_icv_ctrl_related_res = lmer.sex_age_icv_ctrl_related(df_dv = HCP_array_aligned_G2, df_iv = HCP_demographics_cleaned)
lm_G3_sex_age_icv_ctrl_related_res = lmer.sex_age_icv_ctrl_related(df_dv = HCP_array_aligned_G3, df_iv = HCP_demographics_cleaned)
...
write.csv(GSP_lm_G1_res, paste(resdir_gsp, 'R_lm_G1_res.csv', sep = ''), row.names = FALSE)
write.csv(GSP_lm_G2_res, paste(resdir_gsp, 'R_lm_G2_res.csv', sep = ''), row.names = FALSE)
write.csv(GSP_lm_G3_res, paste(resdir_gsp, 'R_lm_G3_res.csv', sep = ''), row.names = FALSE)

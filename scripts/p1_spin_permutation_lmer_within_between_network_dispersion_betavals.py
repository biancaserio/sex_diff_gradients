...
# lists that will contain the permutation beta-values (null distribution of beta-values) for WN and BN dispersion sex contrasts
WN_dispersion_perm_betaval_sex_contrast = []
BN_dispersion_perm_betaval_sex_contrast = []
...
for i in range(len(yeo7_networks_array_perm)):
    ...
    # list that will contain the current permuation's beta values (len 7 because will contain all 7 networks)
    WN_dispersion_perm_betaval_sex_contrast_temp = []
    ...
    for n in range(7):
        ...
        df = pd.DataFrame({'WN_dispersion': scale(sum_of_squares), 'sex': demographics_cleaned_final.Gender, 'age': scale(demographics_cleaned_final.Age_in_Yrs), 'tot_SA': scale(demographics_cleaned_final.tot_SA), 'Family_ID': demographics_cleaned_final.Family_ID, 'TwinStatus': demographics_cleaned_final.TwinStatus})
        ...
        # save results
        WN_dispersion_perm_betaval_sex_contrast_temp.append(results.fe_params[1])
        
        
    # append to permutation results
    WN_dispersion_perm_betaval_sex_contrast.append(WN_dispersion_perm_betaval_sex_contrast_temp)
    ...
    # list that will contain the current permuation's beta values (len 21 because will contain all 21 comparisons of networks)
    BN_dispersion_perm_betaval_sex_contrast_temp = []
    ...
    for n1 in range(7):
        ...
        for n2 in range(7):
            ...
            if n1!=n2 and [n1+1, n2+1] not in pairwise_comparison_order and list(reversed([n1+1, n2+1])) not in pairwise_comparison_order:
                ...
                # make a dataframe that will contain the data for linear mixed effects model (only change is the WN dispersion metric (1-7 networks)
                df = pd.DataFrame({'BN_dispersion': scale(distance), 'sex': demographics_cleaned_final.Gender, 'age': scale(demographics_cleaned_final.Age_in_Yrs), 'tot_SA': scale(demographics_cleaned_final.tot_SA), 'Family_ID': demographics_cleaned_final.Family_ID, 'TwinStatus': demographics_cleaned_final.TwinStatus})
                ...
                # save results
                BN_dispersion_perm_betaval_sex_contrast_temp.append(results.fe_params[1])
                
    
    # append to permutation results
    BN_dispersion_perm_betaval_sex_contrast.append(BN_dispersion_perm_betaval_sex_contrast_temp)
...
print(f'\n---- Export results at /data/p_02667/sex_diff_gradients/results/HCP/SA/WN and BN_dispersion_perm_betaval_sex_contrast_null_distr ----')

# pack into arrays
WN_dispersion_perm_betaval_sex_contrast = np.array(WN_dispersion_perm_betaval_sex_contrast)
BN_dispersion_perm_betaval_sex_contrast = np.array(BN_dispersion_perm_betaval_sex_contrast)


# contains the null distribution of beta values for the sex contrast on the Within Network dispersion per network (7)
WN_dispersion_perm_betaval_sex_contrast_null_distr = {'visual': WN_dispersion_perm_betaval_sex_contrast.T[0],
                                                   'sensory motor': WN_dispersion_perm_betaval_sex_contrast.T[1], 
                                                   'dorsal attention': WN_dispersion_perm_betaval_sex_contrast.T[2], 
                                                   'ventral attention': WN_dispersion_perm_betaval_sex_contrast.T[3], 
                                                   'limbic': WN_dispersion_perm_betaval_sex_contrast.T[4], 
                                                   'fronto parietal': WN_dispersion_perm_betaval_sex_contrast.T[5], 
                                                   'DMN': WN_dispersion_perm_betaval_sex_contrast.T[6]}


# contains the null distribution of beta values for the sex contrast on the Between Network pairwise comparisons (21)
# column names is the pairwise comparison labels (turned into strings)
BN_dispersion_perm_betaval_sex_contrast_null_distr = pd.DataFrame(BN_dispersion_perm_betaval_sex_contrast, columns = [str(pair) for pair in pairwise_comparison_order])  


# export 
pd.DataFrame(WN_dispersion_perm_betaval_sex_contrast_null_distr).to_csv(resdir_hcp+'WN_dispersion_perm_betaval_sex_contrast_null_distr.csv', header = True, index = False)
BN_dispersion_perm_betaval_sex_contrast_null_distr.to_csv(resdir_hcp+'BN_dispersion_perm_betaval_sex_contrast_null_distr.csv', header = True, index = False)
